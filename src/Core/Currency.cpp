...
bool Currency::get_block_reward(uint8_t block_major_version, size_t effective_median_size, size_t current_block_size,
    ...
    {
        base_reward = static_cast<Amount>(parameters::FIXED_REWARD);
    }
...
Difficulty Currency::next_difficultyLWMA2(std::vector<Timestamp> timestamps, std::vector<CumulativeDifficulty> cumulative_difficulties) const
{
    int64_t T = difficulty_target;
    int64_t N = difficulty_window_lwma2 - 1;
    int64_t FTL = block_future_time_limit;
    ...
    //next_D = std::clamp( (prev_D*67)/100, next_D, (prev_D*150)/100 );
    next_D = boost::algorithm::clamp((prev_D*67)/100, next_D, (prev_D*150)/100 );

    if (sum_3_ST < (8 * T) / 10)
        next_D = (prev_D * 108) / 100;

    // Minimum limit
    if (next_D < static_cast<int64_t>(difficulty_limit))
...
bool Currency::is_dust(Amount amount) {
	...
	return pretty_it == std::end(Currency::PRETTY_AMOUNTS) || *pretty_it != amount ||
           //amount < 1000000;  // After fork, dust definition will change
            amount < parameters::DEFAULT_DUST_THRESHOLD;
}
