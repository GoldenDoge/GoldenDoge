...
static const char USAGE[] =
    R"(GoldenDoged )" bytecoin_VERSION_STRING R"(.

Usage:
  GoldenDoged [options]
  GoldenDoged --help | -h (very help)
  GoldenDoged --version | -v (such a version Đ)
...
Options:
  ...
  --GoldenDoged-bind-address=<ip:port>   IP and port for GoldenĐoge RPC API [default: 127.0.0.1:4041].
  ...
  --data-folder=<folder-path>            Folder for blockchain, logs and peer DB [default: )" platform_DEFAULT_DATA_FOLDER_PATH_PREFIX
    R"(GoldenDoge].
  --GoldenDoged-authorization=<usr:pass> HTTP basic authentication credentials for RPC API.)"
#if platform_USE_SSL
    R"(
...
int main(int argc, const char *argv[]) try {
    ...
    platform::ExclusiveLock coin_lock(coin_folder, "GoldenDoged.lock");

	logging::LoggerManager log_manager;
	log_manager.configure_default(config.get_data_folder("logs"), "GoldenDoged-");
    ...
    std::cout << "GoldenDoged started seconds=" << double(idea_ms.count()) / 1000 << std::endl;
	while (!io.stopped()) {
