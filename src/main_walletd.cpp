...
Usage:
  ...
  walletd --help | -h (very help)
  walletd --version | -v (such a version Đ)
...
Options:
  ...
  --walletd-bind-address=<ip:port>      IP and port for walletd RPC API [default: 127.0.0.1:4042].
  --data-folder=<foler-path>            Folder for wallet cache, blockchain, logs and peer DB [default: )" platform_DEFAULT_DATA_FOLDER_PATH_PREFIX
    R"(GoldenDoged].
  --GoldenDoged-remote-address=<ip:port>  Connect to remote GoldenDoged and suppress running built-in GoldenDoged.
  --GoldenDoged-authorization=<user:pass> HTTP basic authentication credentials for RPC API.
  ...
  --net=<main|test>                     Configure for such a mainnet or woow testnet [default: main].

Options for built-in GoldenDoged (run when no --GoldenDoged-remote-address specified):
  --p2p-bind-address=<ip:port>          IP and port for P2P network protocol [default: 0.0.0.0:4040].
  --p2p-external-port=<port>            External port for P2P network protocol, if port forwarding used with NAT [default: 4040].
  --GoldenDoged-bind-address=<ip:port>    		IP and port for GoldenDoged RPC [default: 127.0.0.1:4041].
...
int main(int argc, const char *argv[]) try {
	...
	//	const bool set_password_and_continue  = cmd.get_bool("--set-password-and-continue"); // Run normally after set
	// password, used by GUI wallet
	...
	try {
		if (!config.bytecoind_remote_port)
            blockchain_lock = std::make_unique<platform::ExclusiveLock>(coin_folder, "GoldenDoged.lock");
	} catch (const platform::ExclusiveLock::FailedToLock &ex) {
        std::cout << "GoldenDoged already running - " << ex.what() << std::endl;
	...
	logManagerNode.configure_default(config.get_data_folder("logs"), "GoldenGoded-");
